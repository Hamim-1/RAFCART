const topRankingProduct = [
    [
        {
            name: 'Apple Watch',
            price: '$300.45',
            Dprice: '200.00',
            review: 120,
            img: 'img/watch/watch-1.png'
        },
        {
            name: 'Callie Smartwatch',
            price: '$300.45',
            Dprice: '200.00',
            review: 120,
            img: 'img/watch/watch-2.png'
        },
        {
            name: 'Cruz Smartwatch',
            price: '$300.45',
            Dprice: '200.00',
            review: 120,
            img: 'img/watch/watch-3.png'
        }
    ],
    [
        {
            name: 'Xiaomi Note 7',
            price: '$320.00',
            Dprice: '220.00',
            review: 150,
            img: 'img/phone/phone-1.png'
        },
        {
            name: 'iPhone 11 max',
            price: '$320.00',
            Dprice: '220.00',
            review: 150,
            img: 'img/phone/phone-2.png'
        },
        {
            name: 'Oppo poco f1',
            price: '$320.00',
            Dprice: '220.00',
            review: 150,
            img: 'img/phone/phone-3.png'
        },
    ],
    [
        {
            name: 'Dell inspire 14',
            price: '$1200.00',
            Dprice: '770.00',
            review: 70,
            img: 'img/laptop/laptop-1.png'
        },
        {
            name: 'HP Omen 13',
            price: '$1200.00',
            Dprice: '770.00',
            review: 70,
            img: 'img/laptop/laptop-2.png'
        },
        {
            name: 'HP Pavilion 15',
            price: '$1200.00',
            Dprice: '770.00',
            review: 70,
            img: 'img/laptop/laptop-3.png'
        },
    ],
    [
        {
            name: 'Bose Headphones',
            price: '$95.00',
            Dprice: '75.00',
            review: 90,
            img: 'img/headphone/headphone-1.png'
        },
        {
            name: 'COWIN E7 Active',
            price: '$110.00',
            Dprice: '90.00',
            review: 90,
            img: 'img/headphone/headphone-2.png'
        },
        {
            name: 'Beats Headphones',
            price: '$90.00',
            Dprice: '70.00',
            review: 90,
            img: 'img/headphone/headphone-3.png'
        }

    ]
]

const laptop = [

    {
        name: 'HP Pavilion 15',
        price: '$1200.00',
        Dprice: '770.00',
        review: 70,
        img: 'img/laptop/laptop-3.png'
    },
];

const headphone = [
    {
        name: 'Beats Headphones',
        price: '$120.00',
        Dprice: '70.00',
        review: 90,
        img: 'img/headphone/headphone-3.png'
    }
];

const phone = [
    {
        name: 'Xiaomi Note 7',
        price: '$320.00',
        Dprice: '220.00',
        review: 150,
        img: 'img/phone/phone-1.png'
    },
];

const watch = [];

const monitor = [

    {
        name: 'SAMSUNG 27 Curved',
        price: '$550.00',
        Dprice: '470.00',
        review: 50,
        img: 'img/monitor/hd-monitor.png'
    },
]

const newArrivalsProduct = [laptop, monitor, headphone, phone];
const recomendedProduct = [
    laptop[0],
    {
        name: 'Wireless Mouse',
        price: '$70.00',
        Dprice: '55.00',
        review: 115,
        img: 'img/mouse.png'
    },
    headphone[0],
    phone[0],
    topRankingProduct[1][1],
    topRankingProduct[2][1],
    {
        name: 'Anti-Fray Cloth',
        price: '$30.00',
        Dprice: '22.00',
        review: 315,
        img: 'img/coverpad.png'
    },
    topRankingProduct[3][1]

];

const shopPageGridProduct1 = [
    laptop[0],
    {
        name: 'Men Casual Shoes',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/shoes/shoes-4.png'
    },
    topRankingProduct[3][2],
    {
        name: 'iPhone 12 Pro XR',
        price: '$750.00',
        Dprice: '670.00',
        review: 30,
        img: 'img/phone/iphone-xr.png'
    },
    topRankingProduct[2][1],
    {
        name: 'Women high Shoes',
        price: '$40.00',
        Dprice: '33.00',
        review: 15,
        img: 'img/shoes/shoes-3.png'
    },
    topRankingProduct[1][2],
    topRankingProduct[3][1],
    topRankingProduct[1][1]
];

const shopPageGridProduct2 = [
    laptop[0],
    {
        name: 'Men Casual Shoes',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/shoes/shoes-5.png'
    },
    topRankingProduct[3][1],
    {
        name: 'iPhone 12 Pro XR',
        price: '$750.00',
        Dprice: '670.00',
        review: 30,
        img: 'img/phone/iphone-xr.png'
    },
    topRankingProduct[2][0],
    {
        name: 'Women high Shoes',
        price: '$40.00',
        Dprice: '33.00',
        review: 15,
        img: 'img/shoes/shoes-3.png'
    },
    topRankingProduct[1][1],
    topRankingProduct[3][0],
    topRankingProduct[1][0]
];

const shopPageGridProduct3 = [
    laptop[0],
    {
        name: 'Men Casual Shoes',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/shoes/shoes-5.png'
    },
    topRankingProduct[3][0],
    {
        name: 'iPhone 12 Pro XR',
        price: '$750.00',
        Dprice: '670.00',
        review: 30,
        img: 'img/phone/iphone-xr.png'
    },
    topRankingProduct[2][2],
    {
        name: 'Women high Shoes',
        price: '$40.00',
        Dprice: '33.00',
        review: 15,
        img: 'img/shoes/shoes-3.png'
    },
    topRankingProduct[1][0],
    topRankingProduct[3][2],
    topRankingProduct[1][2]
];

const shopPageListProduct1 = [
    topRankingProduct[3][1],
    topRankingProduct[1][2],
    shopPageGridProduct1[1],
];
const shopPageListProduct2 = [
    topRankingProduct[2][0],
    topRankingProduct[3][0],
    topRankingProduct[1][1],
];
const shopPageListProduct3 = [
    topRankingProduct[3][2],
    topRankingProduct[0][0],
    topRankingProduct[1][0],
];



const recentOrderProduct = [
    {
        img: 'img/tv.png',
        orderNumber: '23E34RT3',
        date: '01 March 2023',
        quantity: 2,
        totalPrice: '$1200',
        status: 'Delivered',
    },
    {
        img: 'img/laptop/laptop-1.png',
        orderNumber: '23E34RT3',
        date: '01 March 2023',
        quantity: 3,
        totalPrice: '$1900',
        status: 'Delivered',
    },
]

const myOrderProduct = [
    {
        img: 'img/tv.png',
        orderNumber: '23E34RT3',
        date: '01 March 2023',
        quantity: 2,
        totalPrice: '$1200',
        status: 'Cancelled',
        color: 'text-red-500'
    },
    {
        img: 'img/headphone/headphone-3.png',
        orderNumber: '23E34RT3',
        date: '01 March 2023',
        quantity: 2,
        totalPrice: '$120',
        status: 'Delivered',
        color: 'text-green-500'
    },
    {
        img: 'img/laptop/laptop-1.png',
        orderNumber: '23E34RT3',
        date: '01 March 2023',
        quantity: 3,
        totalPrice: '$1900',
        status: 'Cancelled',
        color: 'text-red-500'
    },
]


const myReturnProduct = [
    {
        img: 'img/bag-2.png',
        orderNumber: '23E34RT3',
        price: '$40',
        status: 'Successful',
        color: 'text-green-500',
        name: 'Herschel Leather Duffle Bag'
    },
    {
        img: 'img/controller.png',
        orderNumber: '23E34RT3',
        price: '$150',
        status: 'Successful',
        color: 'text-green-500',
        name: 'Xbox One Wireless Controll'
    },
    {
        img: 'img/headphone/headphone-2.png',
        orderNumber: '23E34RT3',
        price: '$120',
        status: 'Processing',
        color: 'text-yellow-500',
        name: 'Sound Intone I65 Earphone'
    },
]


const myCancelledProduct = [
    {
        img: 'img/tv.png',
        orderNumber: '23E34RT3',
        date: '01 March 2023',
        quantity: 2,
        totalPrice: '$800',
        status: 'Cancelled',
        color: 'text-red-500'
    },
    {
        img: 'img/iphone-12.png',
        orderNumber: '23E34RT3',
        date: '01 March 2023',
        quantity: 3,
        totalPrice: '$2700',
        status: 'Cancelled',
        color: 'text-red-500'
    },
    {
        img: 'img/headphone/headphone-1.png',
        orderNumber: '23E34RT3',
        date: '01 March 2023',
        quantity: 1,
        totalPrice: '$100',
        status: 'Cancelled',
        color: 'text-red-500'
    },
]


const myReviewProduct = [
    {
        img: 'img/bag-2.png',
        orderNumber: '23E34RT3',
        price: '$40',
        date: '16 Dec 2020',
        name: 'Herschel Leather Duffle Bag',
        review: 'No',
        buttonText: 'Write Review'
    },
    {
        img: 'img/controller.png',
        orderNumber: '23E34RT3',
        price: '$150',
        date: '16 Dec 2020',
        name: 'Xbox One Wireless Controll',
        review: 'yes',
        buttonText: 'Edit Review'
    },
    {
        img: 'img/headphone/headphone-2.png',
        orderNumber: '23E34RT3',
        price: '$120',
        date: '16 Dec 2020',
        name: 'Sound Intone I65 Earphone',
        review: 'yes',
        buttonText: 'Edit Review'
    },
]


const extendBtnSection1 = {
    height:'h-[197px]',
    arrow:'extended-arr-1',
    extendBtnSection:'my-account-extended-btn-section'
}

const extendBtnSection2 = {
    height:'h-[395px]',
    arrow:'extended-arr-2',
    extendBtnSection:'other-pages-extended-btn-section'
}


const watches = [
    {
        name: 'Watch 1',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/watch/watch-4.png'
    },{
        name: 'Watch 2',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/watch/watch-5.png'
    },{
        name: 'Watch 3',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/watch/watch-6.png'
    },
]

const phones = [
    {
        name: 'Redmi Note 10S',
        price: '$320.00',
        Dprice: '300.00',
        review: 10,
        img: 'img/phone/phone-4.png'
    },{
        name: 'ROG Phone 5',
        price: '$640.00',
        Dprice: '520.00',
        review: 25,
        img: 'img/phone/phone-5.png'
    },{
        name: 'Samsung S22 Ultra',
        price: '$2040.00',
        Dprice: '1900.00',
        review: 20,
        img: 'img/phone/phone-6.png'
    },
]

const headphones = [
    {
        name: 'Headphone 1',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/headphone/headphone-7.png'
    },{
        name: 'Headphone 2',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/headphone/headphone-5.png'
    },{
        name: 'Headphone 3',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/headphone/headphone-6.png'
    },
]

const shoes = [
    {
        name: 'Adidas Shoes',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/shoes/shoes-2.png'
    },{
        name: 'Nike Air Jordan 1',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/shoes/shoes-4.png'
    },{
        name: 'Nike Air Force 1 07',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/shoes/shoes-6.png'
    },{
        name: 'Nike Dunk Low',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/shoes/shoes-7.png'
    }
]

const laptops = [
    {
        name: 'MacBook Air 13',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/laptop/laptop-4.png'
    },{
        name: 'Windows laptop',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/laptop/laptop-5.png'
    },{
        name: 'Gaming laptop',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/laptop/laptop-6.png'
    },{
        name: 'ROG Laptop',
        price: '$40.00',
        Dprice: '20.00',
        review: 60,
        img: 'img/laptop/laptop-7.png'
    }
]

